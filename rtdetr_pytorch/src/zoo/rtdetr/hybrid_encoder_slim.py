'''
HybridEncoder with "Slim P2" strategy but using STANDARD RepVGG blocks.
- P2 (Stride 4): RepVggBlock + SLIM Channels (e.g. 128)
- P3-P5 (Stride 8+): RepVggBlock + Standard Channels (e.g. 256)
'''
...
@register
class HybridEncoderSlim(nn.Module):
    def __init__(self,
                 ...
                 p2_hidden_dim=128):   # Arg for Slim P2
        super().__init__()
        ...
        for idx in range(len(in_channels) - 1, 0, -1):
            ...
            # lateral_conv adapts high channels to low channels
            self.lateral_convs.append(ConvNormLayer(high_dim, low_dim, 1, 1, act=act))
            
            self.fpn_blocks.append(
                CSPRepLayer(low_dim * 2, low_dim, round(3 * depth_mult), act=act, expansion=expansion)
            )
        ...
        for idx in range(len(in_channels) - 1):
            ...
            # downsample adapts low channels to high channels
            self.downsample_convs.append(
                ConvNormLayer(low_dim, high_dim, 3, 2, act=act)
            )
            
            self.pan_blocks.append(
                CSPRepLayer(high_dim * 2, high_dim, round(3 * depth_mult), act=act, expansion=expansion)
            )
    ...
    def forward(self, feats):
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            # same bug fix as HybridEncoderSlimFast
